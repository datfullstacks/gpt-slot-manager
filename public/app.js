...
async function loadAccounts() {
    ...
    tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">Đang tải danh sách accounts...</td></tr>';
    ...
    try {
        ...
        if (accounts.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">Chưa có account nào. Hãy thêm account đầu tiên!</td></tr>';
        ...
        tbody.innerHTML = accounts.map((account, index) => {
            const allowedMembersDisplay = account.allowedMembers && account.allowedMembers.length > 0
                ? account.allowedMembers.map(email => `<span class="member-email">${email}</span>`).join('')
            ...
            return `
                <tr>
                    <td>${index + 1}</td>
                    <td style="font-weight: 500; color: #60a5fa;">${account.email}</td>
                    ...
                    <td>
                        ...
                        <button id="${buttonId}"
                                ...
                                data-allowed-members='${JSON.stringify(account.allowedMembers || [])}'
                    ...
                    <td>
                        <button onclick="showSendInviteModal('${mongoId}', '${account.email}', ${(account.allowedMembers || []).length})" class="btn-table" 
        ...
        setTimeout(() => {
            document.querySelectorAll('.edit-allowed-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    console.log('Edit button clicked:', { accountId, adminEmail, allowedMembers });
        ...
        tbody.innerHTML = '<tr><td colspan="6" class="loading-cell">Lỗi tải danh sách accounts!</td></tr>';
...
async function handleAddAccount(event) {
    ...
    const allowedMembersText = document.getElementById('newAllowedMembers').value.trim();
...
async function deleteMemberFromAccount(accountId, accountUuid, userId, memberEmail) {
    if (!confirm(`Bạn có chắc muốn xóa member "${memberEmail}" khỏi account này?`)) {
        return;
    }
    
    console.log('Deleting member:', { accountId, accountUuid, userId, memberEmail });
    
    try {
        const response = await fetch(`${API_BASE_URL}/accounts/${accountId}/members/${userId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${currentToken}`,
                'Content-Type': 'application/json'
...
async function processAllAccounts() {
    ...
    resultsContainer.innerHTML = '<div class="loading">Đang gọi API từ tất cả accounts...</div>';
    ...
    try {
        ...
        showToast('Lỗi kết nối server!', 'error');
        resultsContainer.innerHTML = '<div class="loading">❌ Lỗi kết nối server!</div>';
                    ...
                    <div class="member-list">
                        ...
                        ${users.map(user => `
                            <div class="member-item">
                                <strong>${user.name || 'N/A'}</strong> - ${user.email || 'N/A'}
                                <br>
                                <small>Role: ${user.role || 'N/A'} | ID: ${user.id || 'N/A'}</small>
                            </div>
                        `).join('')}
                    </div>
...
function updateMembersDisplay(data) {
    ...
    // Update statistics
    document.getElementById('totalAccountsCount').textContent = data.total_accounts;
    ...
    // Show notification
    console.log(`📊 Đã cập nhật: ${successCount}/${data.total_accounts} accounts | ${data.total_members} members`);
}
...
function updateAccountsTable(accounts) {
    ...
    if (accounts.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="loading-cell">Chưa có dữ liệu</td></tr>';
        return;
    }
    
    tbody.innerHTML = accounts.map((account, index) => {
        // Members list provided by backend includes admin as first member (id='admin')
        const membersList = account.members && account.members.length > 0 ? account.members : [];

        const memberEmails = membersList.length > 0
            ? membersList.map(m => {
                ...
                if (m.id === 'admin') {
                    return `
                        <div class="member-item" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px; padding: 4px; border-radius: 4px; background: rgba(255,255,255,0.03);">
                            <span class="member-email" style="flex: 1; font-weight: 600; color: #60a5fa;">${m.email || 'Admin' } (Admin)</span>
                        </div>
                    `;
                }
                ...
                return `
                    <div class="member-item" style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 4px; padding: 4px; border-radius: 4px; background: rgba(255,255,255,0.05);">
                        <span class="member-email" style="flex: 1;">${m.email || m.name || 'N/A'}</span>
        ...
        return `
            <tr>
                <td>${index + 1}</td>
                <td style="font-weight: 500; color: #60a5fa;">${account.email}</td>
                <td style="text-align: center; font-weight: bold; color: ${account.success ? '#10b981' : '#ef4444'};">
                    ${account.members && account.members.length ? account.members.length : 0}
                </td>
                <td>
                ...
                <td>
                    ...
                    ${!account.success ? `<br><small style="color: #ef4444; font-size: 11px;">${account.error || 'Unknown error'}</small>` : ''}
                </td>
                <td>
...
function showEditAllowedMembersModal(accountId, adminEmail, allowedMembers) {
    ...
    document.getElementById('editAllowedMembers').value = (allowedMembers || []).join('\n');
...
async function handleUpdateAllowedMembers(event) {
    ...
    const allowedMembersText = document.getElementById('editAllowedMembers').value.trim();
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/accounts/${accountId}/allowed-members`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${currentToken}`
...
function showSendInviteModal(accountId, adminEmail, currentAllowedMembers) {
    ...
    document.getElementById('remainingSlots').style.color = remaining > 0 ? '#10b981' : '#ef4444';
...
async function handleSendInvite(event) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/accounts/${accountId}/send-invites`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${currentToken}`
        ...
        if (response.ok) {
            showToast(`✅ Đã gửi ${data.invited_count} lời mời! Còn ${data.remaining_slots}/7 slots`, 'success');
            closeSendInviteModal();
            loadAccounts();
        } else {
...
async function sendInvitesToAll() {
    ...
    try {
        ...
        if (response.ok) {
            showToast(`✅ ${data.message} - Tổng ${data.total_invited} lời mời!`, 'success');
...
async function autoCleanupAll() {
    if (!confirm('🧹 TỰ ĐỘNG XÓA tất cả members KHÔNG nằm trong "Member Cố Định"?\n\nCảnh báo: Hành động này không thể hoàn tác!')) {
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            showToast(`✅ ${data.message} - Đã xóa ${data.total_deleted} members!`, 'success');
            ...
            const summary = data.results.map(r => 
                `${r.account}: ${r.deleted || 0} deleted / ${r.unauthorized_found || 0} unauthorized`
            ).join('\n');
